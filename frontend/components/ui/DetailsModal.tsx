...
const DetailsModal: React.FC<DetailsModalProps> = ({ isOpen, onClose, content }) => {
            ...
            <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-3xl shadow-2xl border border-slate-100 overflow-hidden flex flex-col">
                <div className="flex justify-between items-center p-6 border-b border-slate-50">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-slate-900 line-clamp-1">{content.title}</h2>
                            <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Content Details & History</p>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 grid grid-cols-1 lg:grid-cols-2 gap-12">
                    <div className="space-y-8">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                                ...
                                <p className="font-bold text-blue-600">{content.status.replace('_', ' ')}</p>
                            </div>
                            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-6">
                            ...
                            <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest">Approval History</h3>
                        </div>
                ...
                <div className="p-6 border-t border-slate-50 bg-slate-50/50 flex justify-end">
                    <button 
                        onClick={onClose}
                        className="px-6 py-2.5 bg-white border border-slate-200 rounded-xl font-bold text-slate-600 hover:bg-slate-100 transition-colors"
                    >
                        Close
                    </button>
                </div>
