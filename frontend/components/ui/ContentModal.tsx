...
const ContentModal: React.FC<ContentModalProps> = ({ isOpen, onClose, onSuccess, content }) => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
            <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl border border-slate-100 overflow-hidden">
                <div className="flex justify-between items-center p-6 border-b border-slate-50">
                    <h2 className="text-xl font-bold text-slate-900">{content ? 'Edit Content' : 'Create New Content'}</h2>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div className="flex gap-3 pt-4">
                        ...
                        <button 
                            type="submit" 
                            disabled={loading}
                            className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold transition-all flex items-center justify-center disabled:opacity-70"
                        >
                            {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : (content ? 'Update Content' : 'Create Content')}
                        </button>
                    </div>
                </form>
            </div>
