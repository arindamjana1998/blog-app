...
class ContentService {
    ...
    async updateContent(id, data, userId, userRole) {
        ...
        if (userRole === 'creator') {
            ...
            if (!['draft', 'rejected'].includes(content.status)) {
                throw new AppError('You can only edit content if it is in draft or rejected status', 400);
            }
        }
    ...
    async submitContent(id, userId, userRole) {
        ...
        if (userRole === 'creator' && content.createdBy._id.toString() !== userId.toString()) {
    ...
    async approveContent(id, userId, userRole, comment) {
        ...
        if (content.status === 'pending_review_level_1') {
            ...
            const l1Review = content.approvalHistory.find(h => h.step === 1 && h.action === 'APPROVED');
            if (userRole !== 'admin' && l1Review && l1Review.actedBy.toString() === userId.toString()) {
                throw new AppError('You cannot perform Level 2 approval as you were the Level 1 reviewer', 400);
            }
    ...
    async rejectContent(id, userId, userRole, comment) {
        ...
        if (!['pending_review_level_1', 'pending_review_level_2'].includes(content.status)) {
    ...
    async publishContent(id, userId, userRole) {
        ...
        if (!['admin', 'reviewer'].includes(userRole)) {
            throw new AppError('Only administrators or reviewers can publish content', 403);
        }
